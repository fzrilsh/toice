...
- **Phase 4.5 (code complete, pure Dart).** UX polish + app-layer Trip PIN auth. Renamed `nonce` to a 4-6 digit numeric Trip PIN everywhere (Dart + JSON key, no migration, unreleased MVP). App-layer HMAC-SHA256 challenge/response over the mock `Signaling` channel (`lib/audio/auth.dart`): host admits a client only if it proves it knows the PIN, closing the Option-B gap where anyone who scanned the `WIFI:` QR was on the LAN. PIN-only `TOICE-PIN:` QR for optional Android auto-fill; PIN entry screen; high-contrast outdoor dark theme; navigation transitions + surfaced join errors (`lib/ui/`). `crypto` pinned exact. Auth is HMAC over a low-entropy PIN, not a PAKE (ponytail note, resists capture/replay not active brute force); still on the mock session (real audio at Phase 6).
